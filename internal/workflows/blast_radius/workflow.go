...
type Result struct {
	QueryResult  queryport.BlastRadiusResult `json:"query_result"`
...
func (w Workflow) Run(req Request) (Result, error) {
	result, err := w.queryService.BlastRadius(queryport.BlastRadiusRequest{
	...
	refs := []artifact.Ref{}
	if ref, err := w.artifactStore.SaveJSON(req.WorkspaceID, req.SnapshotID, "blast_radius.json", artifact.TypeBlastRadius, result); err == nil {
