...
// Mode controls the progress reporter output style.
type Mode string
...
// NoopReporter is a Reporter that discards all output.
...
// StderrReporter writes progress output to stderr.
...
// NewProgressReporter returns a Reporter configured with the given mode.
// If mode is empty or "auto", it chooses tty/plain based on whether stderr is a terminal.
func NewProgressReporter(mode ...string) Reporter {
	m := string(ModeAuto)
	if len(mode) > 0 {
		m = mode[0]
	}
	resolved := normalizeMode(m, isTerminal(os.Stderr))
