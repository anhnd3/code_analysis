...
func (s *SQLiteStore) SaveIndex(index Index) error {
	...
	if _, err := tx.Exec(`
	insert or replace into index_runs(workspace_id, snapshot_id, generated_at, repository_count, file_count, symbol_count, call_candidate_count, issue_counts_json)
	values(?, ?, ?, ?, ?, ?, ?, ?)
`,
		index.WorkspaceID,
		index.SnapshotID,
...
insert or replace into repositories(workspace_id, snapshot_id, id, name, root_path, role, languages_json, build_files_json, frameworks_json)
values(?, ?, ?, ?, ?, ?, ?, ?, ?)
`)
	if err != nil {
		return err
...
insert or replace into services(workspace_id, snapshot_id, id, repository_id, name, root_path, entrypoints_json, boundary_hints_json)
values(?, ?, ?, ?, ?, ?, ?, ?)
`)
	if err != nil {
		return err
...
insert or replace into files(workspace_id, snapshot_id, id, repository_id, repository_root, relative_path, absolute_path, language, package_name)
values(?, ?, ?, ?, ?, ?, ?, ?, ?)
`)
	if err != nil {
		return err
...
insert or replace into symbols(workspace_id, snapshot_id, id, repository_id, file_id, file_path, canonical_name, name, receiver, kind, signature, start_line, start_col, end_line, end_col, start_byte, end_byte)
values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`)
	if err != nil {
		return err
...
insert or replace into imports(workspace_id, snapshot_id, id, file_id, file_path, source, alias, imported_name, export_name, resolved_path, is_default, is_namespace, is_local)
values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`)
	if err != nil {
		return err
...
insert or replace into exports(workspace_id, snapshot_id, id, file_id, file_path, name, canonical_name, is_default)
values(?, ?, ?, ?, ?, ?, ?, ?)
`)
	if err != nil {
		return err
...
insert or replace into call_candidates(workspace_id, snapshot_id, id, source_symbol_id, target_symbol_id, target_canonical_name, target_file_path, target_export_name, relationship, evidence_type, evidence_source, extraction_method, confidence_score, order_index)
values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`)
	if err != nil {
		return err
...
insert or replace into execution_hints(workspace_id, snapshot_id, id, file_path, source_symbol_id, target_symbol_id, target_symbol, kind, evidence, order_index)
values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`)
	if err != nil {
		return err
...
insert or replace into diagnostics(workspace_id, snapshot_id, id, file_path, symbol_id, category, message, evidence)
values(?, ?, ?, ?, ?, ?, ?, ?)
`)
	if err != nil {
		return err
...
insert or replace into tests(workspace_id, snapshot_id, id, symbol_id, file_id, file_path, canonical_name, name, start_line, end_line)
values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`)
	if err != nil {
		return err
...
insert or replace into boundaries(workspace_id, snapshot_id, id, repository_id, kind, framework, method, path, canonical_name, source_file, source_expr, handler_target, source_start_byte, source_end_byte)
values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`)
	if err != nil {
		return err
...
func (s *SQLiteStore) ResolveSymbol(workspaceID, snapshotID, selector string) (SymbolFact, error) {
	row := s.db.QueryRow(`
...
func (s *SQLiteStore) GetSymbolByID(workspaceID, snapshotID, symbolID string) (SymbolFact, error) {
	row := s.db.QueryRow(`
select id, repository_id, file_id, file_path, canonical_name, name, receiver, kind, signature, start_line, start_col, end_line, end_col, start_byte, end_byte
from symbols
...
func (s *SQLiteStore) GetFileByID(workspaceID, snapshotID, fileID string) (FileFact, error) {
	row := s.db.QueryRow(`
...
func (s *SQLiteStore) ListImportsByFile(workspaceID, snapshotID, fileID string) ([]ImportFact, error) {
	rows, err := s.db.Query(`
...
func (s *SQLiteStore) ListOutgoingCallCandidates(workspaceID, snapshotID, sourceSymbolID string) ([]CallCandidate, error) {
	rows, err := s.db.Query(`
...
func (s *SQLiteStore) ListIncomingCallCandidates(workspaceID, snapshotID, targetSymbolID string) ([]CallCandidate, error) {
	rows, err := s.db.Query(`
select id, source_symbol_id, target_symbol_id, target_canonical_name, target_file_path, target_export_name, relationship, evidence_type, evidence_source, extraction_method, confidence_score, order_index
from call_candidates
...
func (s *SQLiteStore) ListTestsByFile(workspaceID, snapshotID, fileID string) ([]TestFact, error) {
	rows, err := s.db.Query(`
...
func (s *SQLiteStore) SaveReviewFlow(flow ReviewFlow) error {
	_, err := s.db.Exec(`
...
func (s *SQLiteStore) GetReviewFlow(flowID string) (ReviewFlow, error) {
	row := s.db.QueryRow(`
...
func (s *SQLiteStore) deleteSnapshot(tx *sql.Tx, workspaceID, snapshotID string) error {
	tables := []string{
		"review_flows",
		"boundaries",
		"tests",
...
func scanCallCandidates(rows *sql.Rows) ([]CallCandidate, error) {
	var out []CallCandidate
	for rows.Next() {
		var item CallCandidate
		if err := rows.Scan(
			&item.ID,
			&item.SourceSymbolID,
			&item.TargetSymbolID,
...
func scanSymbol(row scanner) (SymbolFact, error) {
	var symbolFact SymbolFact
	if err := row.Scan(
		&symbolFact.ID,
