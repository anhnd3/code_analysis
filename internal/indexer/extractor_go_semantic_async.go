...
//   - wg.Wait()     -> HintWait
func extractAsyncHints(node *tree_sitter.Node, content []byte, src Symbol, env goCallEnv, syntheticIndex syntheticSpanIndex) []semanticHintMatch {
	...
	walk(node, func(inner *tree_sitter.Node) bool {
		...
		switch inner.Kind() {
		case "go_statement":
			if hint, ok := asyncHint(inner, content, src, env, syntheticIndex, HintSpawn); ok {
				hints = append(hints, hint)
			}

		case "defer_statement":
			if hint, ok := asyncHint(inner, content, src, env, syntheticIndex, HintDefer); ok {
				hints = append(hints, hint)
			}
...
func asyncHint(node *tree_sitter.Node, content []byte, src Symbol, env goCallEnv, syntheticIndex syntheticSpanIndex, kind HintKind) (semanticHintMatch, bool) {
