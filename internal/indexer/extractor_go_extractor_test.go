...
func TestExtractorParsesGoFixture(t *testing.T) {
	...
	for _, sym := range result.Symbols {
		if sym.Name == "Handle" && sym.Kind == SymbolKindFunction {
			sawHandle = true
		}
		if sym.Name == "stepOne" && sym.Kind == SymbolKindFunction {
			sawStepOne = true
		}
	}
...
func TestExtractorFindsTestFunctions(t *testing.T) {
	...
	for _, sym := range result.Symbols {
		if sym.Name == "TestHandle" && sym.Kind == SymbolKindTestFunction {
			sawTest = true
		}
	}
	for _, rel := range result.Relations {
...
`))
	...
	result, err := extractor.ExtractFile(FileRef{
		RepositoryID:   "demo",
	...
	assertRelationTarget(t, result.Relations, "h.helper", "handler.handler.helper", TargetKind(TargetRefKindExactCanonical), "receiver_selector")
	assertRelationTarget(t, result.Relations, "h.repo.DetectQR", "repo.DetectQR", TargetKind(TargetRefKindPackageMethodHint), "receiver_field_selector")
	assertRelationTarget(t, result.Relations, "h.session.GetSessionByZlpToken", "session.GetSessionByZlpToken", TargetKind(TargetRefKindPackageMethodHint), "receiver_field_selector")
	assertRelationTarget(t, result.Relations, "fmt.Println", "fmt.Println", TargetKind(TargetRefKindExactCanonical), "import_selector")
...
func assertRelationTarget(t *testing.T, relations []RelationCandidate, evidenceSource, target string, kind TargetKind, evidenceType string) {
	t.Helper()
...
func assertRelationOrder(t *testing.T, relations []RelationCandidate, evidenceSource string, orderIndex int) {
	t.Helper()
	for _, relation := range relations {
		if relation.EvidenceSource != evidenceSource {
			continue
