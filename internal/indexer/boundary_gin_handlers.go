...
func indexGinPackageSymbols(state *ginPackageState, symbols []Symbol) {
	if state == nil {
		return
	}
	for _, sym := range symbols {
		...
		if state.packageMethods[sym.Receiver] == nil {
			state.packageMethods[sym.Receiver] = map[string][]Symbol{}
		}
		state.packageMethods[sym.Receiver][sym.Name] = append(state.packageMethods[sym.Receiver][sym.Name], sym)
	}
}

func isGinExecutableSymbol(sym Symbol) bool {
	switch sym.Kind {
	case SymbolKindFunction, SymbolKindMethod, SymbolKindRouteHandler, SymbolKindGRPCHandler:
		return true
	default:
		return false
	}
...
func (d *GinDetector) bindHandlerBindings(stmt *tree_sitter.Node, file ParsedGoFile, imports map[string]string, handlerEnv map[string]handlerBinding, receiverEnv map[string]string, state *ginPackageState) bool {
	switch stmt.Kind() {
...
func (d *GinDetector) bindHandlerAssignment(stmt *tree_sitter.Node, file ParsedGoFile, imports map[string]string, handlerEnv map[string]handlerBinding, receiverEnv map[string]string, state *ginPackageState) bool {
...
func (d *GinDetector) bindHandlerVarSpec(spec *tree_sitter.Node, file ParsedGoFile, imports map[string]string, handlerEnv map[string]handlerBinding, receiverEnv map[string]string, state *ginPackageState) bool {
...
func (d *GinDetector) captureHandlerProviderReturn(stmt *tree_sitter.Node, file ParsedGoFile, imports map[string]string, handlerEnv map[string]handlerBinding, receiverEnv map[string]string, receiverType string, state *ginPackageState) bool {
...
func (d *GinDetector) resolveAssignedHandlerBinding(expr *tree_sitter.Node, file ParsedGoFile, imports map[string]string, handlerEnv map[string]handlerBinding, receiverEnv map[string]string, state *ginPackageState) (handlerBinding, bool) {
...
func (d *GinDetector) resolvePreparedHandlerBindingExpression(expr *tree_sitter.Node, file ParsedGoFile, imports map[string]string, handlerEnv map[string]handlerBinding, receiverEnv map[string]string, state *ginPackageState, providerDepth int) (handlerBinding, bool) {
	expr = unwrapParens(expr)
	if expr == nil {
		return handlerBinding{}, false
	}
	...
	case "composite_literal":
		...
		return handlerBinding{
			packageToken: NormalizePackageToken(file.PackageName),
	...
	case "call_expression":
		...
		if importAliasMatchesFunc(imports, nodeText(operand, file.Content), func(string) bool { return true }) {
			...
			if looksLikeHandlerConstructor(methodName) {
				return handlerBinding{packageToken: NormalizePackageToken(nodeText(operand, file.Content))}, true
			}
		}
...
func (s *ginPackageState) uniqueFunction(name string) (Symbol, bool) {
	if s == nil {
		return Symbol{}, false
	}
...
func (s *ginPackageState) uniqueMethod(receiverType, methodName string) (Symbol, bool) {
	if s == nil {
		return Symbol{}, false
	}
...
func (d *GinDetector) resolveGinHandlerTarget(handlerArg *tree_sitter.Node, file ParsedGoFile, symbols []Symbol, handlerEnv map[string]handlerBinding, receiverEnv map[string]string, state *ginPackageState) (string, TargetRefKind) {
	if symbolID, ok := exactSymbolIDAtNode(handlerArg, symbols); ok {
		return symbolID, TargetRefKindExactSymbolID
	}

	handlerArg = unwrapParens(handlerArg)
	if handlerArg == nil {
		return "", TargetRefKindUnknown
	}

	switch handlerArg.Kind() {
	case "identifier":
		if sym, ok := state.uniqueFunction(nodeText(handlerArg, file.Content)); ok {
			return sym.CanonicalName, TargetRefKindExactCanonical
		}
...
func exactSymbolIDAtNode(node *tree_sitter.Node, symbols []Symbol) (string, bool) {
	if node == nil {
		return "", false
	}
	for _, sym := range symbols {
...
func (d *GinDetector) resolveGinSelectorTarget(selector *tree_sitter.Node, file ParsedGoFile, handlerEnv map[string]handlerBinding, receiverEnv map[string]string, state *ginPackageState) (string, TargetRefKind) {
	...
	if binding.receiverType != "" && NormalizePackageToken(binding.packageToken) == NormalizePackageToken(file.PackageName) {
		if sym, ok := state.uniqueMethod(binding.receiverType, methodName); ok {
			return sym.CanonicalName, TargetRefKindExactCanonical
		}
	}
	hint := BuildPackageMethodHint(binding.packageToken, methodName)
	if hint == "" {
		return "", TargetRefKindUnknown
	}
	return hint, TargetRefKindPackageMethodHint
}
...
func ginLocalReceiverType(expr *tree_sitter.Node, content []byte, currentPackage string, handlerEnv map[string]handlerBinding, receiverEnv map[string]string) (string, bool) {
	...
	case "identifier":
		...
		if binding, ok := handlerEnv[name]; ok && NormalizePackageToken(binding.packageToken) == NormalizePackageToken(currentPackage) && binding.receiverType != "" {
