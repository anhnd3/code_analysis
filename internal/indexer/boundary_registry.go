...
func diagnosticKey(diag Diagnostic) string {
...
func dedupKey(root BoundaryRoot) string {
	return fmt.Sprintf(
		"%s|%s|%d|%d|%s|%s|%s",
		root.RepositoryID,
		root.SourceFile,
...
func (r *Registry) PreparePackage(files []ParsedGoFile, symbols []Symbol) []Diagnostic {
...
func (r *Registry) DetectAllDetailed(file ParsedGoFile, symbols []Symbol) ([]Result, []Diagnostic) {
	// Collect raw results from all detectors.
	var raw []registryEntry
	diagByKey := map[string]Diagnostic{}
	...
	diagsByKey := make(map[string][]Diagnostic, len(raw))
	...
	for _, e := range raw {
		...
		if resultWins(e, existing) {
			...
			diagsByKey[e.key] = append(diagsByKey[e.key], Diagnostic{
...
func (r *Registry) DetectAll(file ParsedGoFile, symbols []Symbol) []Result {
