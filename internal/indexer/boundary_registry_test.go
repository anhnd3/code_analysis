...
func (s *stubDetector) DetectBoundaries(_ ParsedGoFile, _ []Symbol) ([]BoundaryRoot, []Diagnostic) {
	return s.roots, nil
}

func makeRoot(repoID, sourceFile, method, path, handler, confidence, framework string, start, end uint32) BoundaryRoot {
	root := BoundaryRoot{
		Kind:            RootKindHTTP,
		Framework:       framework,
		Method:          method,
...
func TestDetectAll_DeduplicatesIdenticalRootKey(t *testing.T) {
	...
	r := NewRegistry()
	r.Register(&stubDetector{name: "gin", roots: []BoundaryRoot{root}})
	r.Register(&stubDetector{name: "net/http", roots: []BoundaryRoot{dup}})
...
func TestDetectAll_DistinctSpansDoNotCollide(t *testing.T) {
	...
	r := NewRegistry()
	r.Register(&stubDetector{name: "gin", roots: []BoundaryRoot{first, second}})
...
func TestDetectAll_DeterministicWinnerSelection(t *testing.T) {
	...
	r := NewRegistry()
	r.Register(&stubDetector{name: "net/http", roots: []BoundaryRoot{netHTTPRoot}})
	r.Register(&stubDetector{name: "gin", roots: []BoundaryRoot{ginRoot}})
