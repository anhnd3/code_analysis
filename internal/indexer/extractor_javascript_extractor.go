...
func NewJavaScriptExtractor() SymbolExtractor {
	return &JavaScriptExtractor{
		jsParser:  NewJavaScriptParser(),
		tsParser:  NewTypeScriptParser(),
		tsxParser: NewTSXParser(),
	}
}

func (e *JavaScriptExtractor) Supports(lang string) bool {
	return strings.EqualFold(lang, "javascript") || strings.EqualFold(lang, "typescript")
}

func (e *JavaScriptExtractor) ExtractFile(file FileRef) (FileExtractionResult, error) {
	...
	result.Imports = jsCollectImportSources(imports)
...
func (e *JavaScriptExtractor) parse(file FileRef, content []byte) (*tree_sitter.Tree, error) {
...
func collectJSSymbols(file FileRef, content []byte, node *tree_sitter.Node, moduleName, className string, result *FileExtractionResult, contexts *[]jsSymbolContext) {
	if node == nil {
		return
	}
	switch node.Kind() {
	case "class_declaration":
		...
		sym := buildJSSymbol(file, moduleName, name, "", SymbolKindClass, node, content)
	...
	case "call_expression":
		...
		sym := buildJSSymbol(file, moduleName, syntheticName, "", SymbolKindTestFunction, callback, content)
		result.Symbols = append(result.Symbols, sym)
		*contexts = append(*contexts, jsSymbolContext{symbol: sym, body: bodyNode})
		return
	}
...
func buildJSSymbol(file FileRef, moduleName, name, receiver string, kind SymbolKind, node *tree_sitter.Node, content []byte) Symbol {
	return Symbol{
		...
		Location:      jsLocationFromNode(file.RelativePath, node),
	}
}

func extractJSImportBindings(file FileRef, content []byte, root *tree_sitter.Node) ([]ImportBinding, map[string]ImportBinding) {
	imports := make([]ImportBinding, 0, 8)
	byAlias := map[string]ImportBinding{}
	jsWalk(root, func(node *tree_sitter.Node) {
		...
		case strings.HasPrefix(clause, "{"):
			for _, part := range jsSplitCommaSeparated(strings.Trim(clause, "{}")) {
		...
		default:
			...
			if strings.Contains(clause, ",") {
				parts := jsSplitCommaSeparated(clause)
				if len(parts) > 0 {
...
func collectJSExports(moduleName string, content []byte, root *tree_sitter.Node, symbols []Symbol) []ExportBinding {
	exports := make([]ExportBinding, 0, 8)
	...
	jsWalk(root, func(node *tree_sitter.Node) {
			...
			if declaration := firstNamedChild(node); declaration != nil {
				...
				if nameNode != nil {
					...
					if sym, ok := symbolByName[name]; ok {
						exports = append(exports, ExportBinding{Name: "default", CanonicalName: sym.CanonicalName, IsDefault: true})
					}
				}
			}
		...
		case strings.HasPrefix(text, "export {"):
			for _, part := range jsSplitCommaSeparated(strings.Trim(strings.TrimPrefix(strings.TrimSuffix(text, ";"), "export "), "{}")) {
				fields := strings.SplitN(part, " as ", 2)
				...
				if sym, ok := symbolByName[name]; ok {
					exports = append(exports, ExportBinding{Name: exportName, CanonicalName: sym.CanonicalName, IsDefault: exportName == "default"})
				}
			}
		default:
			if declaration := firstNamedChild(node); declaration != nil {
				...
				if sym, ok := symbolByName[name]; ok {
					exports = append(exports, ExportBinding{Name: name, CanonicalName: sym.CanonicalName})
				} else {
					exports = append(exports, ExportBinding{Name: name, CanonicalName: jsCanonicalName(moduleName, "", name)})
				}
			}
		}
	})
...
func extractJSRelations(content []byte, moduleName string, ctx jsSymbolContext, imports map[string]ImportBinding, result *FileExtractionResult) {
	jsWalk(ctx.body, func(node *tree_sitter.Node) {
...
func resolveJSCall(moduleName string, ctx jsSymbolContext, node *tree_sitter.Node, content []byte, imports map[string]ImportBinding) (RelationCandidate, *Diagnostic) {
	switch node.Kind() {
	case "identifier":
		...
		if binding, ok := imports[name]; ok {
			if !binding.IsLocal || binding.ResolvedPath == "" {
				return RelationCandidate{}, &Diagnostic{
...
func dedupeImportBindings(bindings []ImportBinding) []ImportBinding {
	seen := map[string]ImportBinding{}
	for _, binding := range bindings {
		key := binding.Source + "|" + binding.Alias + "|" + binding.ImportedName + "|" + binding.ResolvedPath + "|" + binding.ExportName
		seen[key] = binding
	}
	result := make([]ImportBinding, 0, len(seen))
...
func dedupeExportBindings(bindings []ExportBinding) []ExportBinding {
	seen := map[string]ExportBinding{}
	for _, binding := range bindings {
		key := binding.Name + "|" + binding.CanonicalName
		seen[key] = binding
	}
	result := make([]ExportBinding, 0, len(seen))
	for _, binding := range seen {
		result = append(result, binding)
	}
	sort.Slice(result, func(i, j int) bool {
...
func jsCollectImportSources(bindings []ImportBinding) []string {
...
func jsSplitCommaSeparated(raw string) []string {
...
func detectJSTestKind(path, name string) SymbolKind {
	lower := strings.ToLower(filepath.Base(path))
	if strings.Contains(lower, ".test.") || strings.Contains(lower, ".spec.") || strings.HasPrefix(strings.ToLower(name), "test") {
		return SymbolKindTestFunction
	}
	return SymbolKindFunction
}

func jsLocationFromNode(path string, node *tree_sitter.Node) CodeLocation {
...
func jsWalk(node *tree_sitter.Node, visit func(*tree_sitter.Node)) {
	if node == nil {
		return
	}
	visit(node)
	for i := uint(0); i < node.NamedChildCount(); i++ {
		jsWalk(node.NamedChild(i), visit)
	}
}
