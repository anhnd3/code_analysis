...
func GenerateClosureSymbol(file FileRef, pkg string, parentCanonicalName string, index int, node *tree_sitter.Node) Symbol {
	syntheticName := fmt.Sprintf("$closure_return_%d", index)
	canonical := fmt.Sprintf("%s.%s", parentCanonicalName, syntheticName)

	return Symbol{
		ID:            SymbolID(StableID("synthetic", file.RepositoryID, file.RelativePath, canonical)),
...
func GenerateInlineSymbol(file FileRef, pkg string, callContextCanonical string, index int, node *tree_sitter.Node) Symbol {
