...
type ginPackageState struct {
	...
	packageFunctions       map[string][]Symbol
	packageMethods         map[string]map[string][]Symbol
...
func (d *GinDetector) PreparePackage(files []ParsedGoFile, symbols []Symbol) []Diagnostic {
	if len(files) == 0 {
		return nil
	}
...
func (d *GinDetector) DetectBoundaries(file ParsedGoFile, symbols []Symbol) ([]BoundaryRoot, []Diagnostic) {
	var roots []BoundaryRoot
...
func (d *GinDetector) prepareFile(file ParsedGoFile, state *ginPackageState) bool {
	changed := false
	imports := fileImportAliases(file)
	walk(file.Root, func(n *tree_sitter.Node) bool {
		switch n.Kind() {
...
func (d *GinDetector) prepareBlock(block *tree_sitter.Node, file ParsedGoFile, imports map[string]string, inheritedScope map[string]ginContext, inheritedHandlers map[string]handlerBinding, inheritedReceivers map[string]string, receiverType string, state *ginPackageState) bool {
...
func (d *GinDetector) processBlock(block *tree_sitter.Node, file ParsedGoFile, symbols []Symbol, imports map[string]string, inheritedScope map[string]ginContext, inheritedHandlers map[string]handlerBinding, inheritedReceivers map[string]string, state *ginPackageState, helperDepth int, helperStack map[string]bool) ([]BoundaryRoot, []Diagnostic) {
	scope := cloneGinScope(inheritedScope)
	handlerEnv := cloneHandlerBindings(inheritedHandlers)
	receiverEnv := cloneReceiverAliases(inheritedReceivers)
	var roots []BoundaryRoot
...
func (d *GinDetector) handleRouteCall(call *tree_sitter.Node, file ParsedGoFile, scope map[string]ginContext, handlerEnv map[string]handlerBinding, receiverEnv map[string]string, state *ginPackageState, symbols []Symbol) (*BoundaryRoot, Diagnostic) {
	...
	root := BoundaryRoot{
		...
		HandlerTargetKind: TargetKind(handlerTargetKind),
...
func (d *GinDetector) expandHelperCall(call *tree_sitter.Node, file ParsedGoFile, imports map[string]string, scope map[string]ginContext, handlerEnv map[string]handlerBinding, receiverEnv map[string]string, state *ginPackageState, helperDepth int, helperStack map[string]bool) ([]BoundaryRoot, []Diagnostic) {
...
func (d *GinDetector) resolveHelperCallable(call *tree_sitter.Node, file ParsedGoFile, handlerEnv map[string]handlerBinding, receiverEnv map[string]string, state *ginPackageState) (ginCallable, bool) {
...
func resolveHandlerTarget(handlerArg *tree_sitter.Node, content []byte, symbols []Symbol) string {
	if handlerArg == nil {
		return ""
	}
...
func (d *GinDetector) packageKey(file ParsedGoFile) string {
...
func filePackageToken(file ParsedGoFile) string {
	return normalizeTypeName(file.PackageName)
}

func indexGinPackageCallables(state *ginPackageState, files []ParsedGoFile) {
	if state == nil {
		return
	}
	for _, file := range files {
...
func boundaryDiagnostic(category, filePath, evidence, message string) Diagnostic {
	return Diagnostic{
		Category: category,
		FilePath: filePath,
		Evidence: evidence,
...
func sortGinDiagnostics(diags []Diagnostic) []Diagnostic {
...
func dedupeGinRoots(roots []BoundaryRoot) []BoundaryRoot {
	seen := map[string]BoundaryRoot{}
	...
	out := make([]BoundaryRoot, 0, len(seen))
