...
func NewPythonExtractor() SymbolExtractor {
	return &Extractor{parser: NewPythonParser()}
}
...
func (e *Extractor) ExtractFile(file FileRef) (FileExtractionResult, error) {
	...
	result.Imports = pyCollectImportSources(imports)
...
func collectPythonSymbols(file FileRef, content []byte, node *tree_sitter.Node, moduleName, className string, inFunction bool, result *FileExtractionResult, contexts *[]pythonSymbolContext) {
	if node == nil {
		return
	}
	switch node.Kind() {
	case "class_definition":
		...
		sym := buildPythonSymbol(file, moduleName, name, "", SymbolKindClass, node, content)
		result.Symbols = append(result.Symbols, sym)
		if !inFunction {
			result.Exports = append(result.Exports, ExportBinding{Name: name, CanonicalName: sym.CanonicalName})
		}
...
func buildPythonSymbol(file FileRef, moduleName, name, receiver string, kind SymbolKind, node *tree_sitter.Node, content []byte) Symbol {
	return Symbol{
...
func extractPythonImports(file FileRef, content []byte, root *tree_sitter.Node) ([]ImportBinding, map[string]ImportBinding) {
	imports := make([]ImportBinding, 0, 8)
	byAlias := map[string]ImportBinding{}
	pyWalk(root, func(node *tree_sitter.Node) {
		switch node.Kind() {
		case "import_statement":
			statement := strings.TrimSpace(node.Utf8Text(content))
			statement = strings.TrimPrefix(statement, "import ")
			for _, part := range pySplitCommaSeparated(statement) {
		...
		case "import_from_statement":
			...
			resolvedPath, local := resolvePythonModulePath(file.RepositoryRoot, file.RelativePath, moduleSpec)
			for _, part := range pySplitCommaSeparated(strings.Trim(importSection, "()")) {
				fields := strings.SplitN(part, " as ", 2)
	...
	return pyDedupeImportBindings(imports), byAlias
}

func extractPythonRelations(content []byte, moduleName string, ctx pythonSymbolContext, imports map[string]ImportBinding, result *FileExtractionResult) {
	pyWalk(ctx.body, func(node *tree_sitter.Node) {
...
func resolvePythonCall(moduleName string, ctx pythonSymbolContext, node *tree_sitter.Node, content []byte, imports map[string]ImportBinding) (RelationCandidate, *Diagnostic) {
	switch node.Kind() {
	case "identifier":
...
func pyDedupeImportBindings(bindings []ImportBinding) []ImportBinding {
...
func pyCollectImportSources(bindings []ImportBinding) []string {
...
func pySplitCommaSeparated(raw string) []string {
...
func locationFromNode(path string, node *tree_sitter.Node) CodeLocation {
...
func pyWalk(node *tree_sitter.Node, visit func(*tree_sitter.Node)) {
	if node == nil {
		return
	}
	visit(node)
	for i := uint(0); i < node.NamedChildCount(); i++ {
		pyWalk(node.NamedChild(i), visit)
	}
}
