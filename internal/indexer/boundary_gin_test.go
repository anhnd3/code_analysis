...
func main() {
	...
	packageSymbols := []Symbol{
		{ID: "sym_health", RepositoryID: "repo1", FilePath: "main.go", PackageName: "main", Name: "Health", Receiver: "handlers", CanonicalName: "main.handlers.Health", Kind: SymbolKindMethod},
		{ID: "sym_detect_v1", RepositoryID: "repo1", FilePath: "main.go", PackageName: "main", Name: "DetectQR", Receiver: "handlers", CanonicalName: "main.handlers.DetectQR", Kind: SymbolKindMethod},
		{ID: "sym_detect_v2", RepositoryID: "repo1", FilePath: "main.go", PackageName: "main", Name: "DetectQRV2", Receiver: "handlers", CanonicalName: "main.handlers.DetectQRV2", Kind: SymbolKindMethod},
	}
	...
	for _, root := range roots {
		...
		if root.ID == "" || root.ID != StableBoundaryID(root) {
...
func (s *service) withRouter() {
	...
	for _, canonical := range required {
		...
		if root.ID != StableBoundaryID(root) {
...
func parseGinFiles(t *testing.T, files map[string][]byte) []ParsedGoFile {
	t.Helper()
	...
	for path, source := range files {
		...
		parsed = append(parsed, ParsedGoFile{
			RepositoryID: "repo1",
			Path:         path,
	...
	slices.SortFunc(parsed, func(left, right ParsedGoFile) int {
		switch {
...
func rootByCanonicalName(t *testing.T, roots []BoundaryRoot, canonical string) BoundaryRoot {
	t.Helper()
	for _, root := range roots {
...
func collectDiagnosticCategories(diags []Diagnostic) []string {
...
func describeFunctionBodies(files []ParsedGoFile) string {
