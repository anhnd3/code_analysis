...
func (ServiceDetector) Detect(repo Manifest, policy IgnorePolicy) ([]ServiceManifest, []BoundaryHint, error) {
	walkResult, err := Walk(repo.RootPath, policy, nil)
	if err != nil {
		return nil, nil, err
	}

	hints := make([]BoundaryHint, 0, 8)
	candidates := map[string]*ServiceManifest{}
	for _, entry := range walkResult.Entries {
		...
		case strings.HasSuffix(lower, ".proto"):
			hints = append(hints, BoundaryHint{Type: "grpc", Path: rel, Details: "proto definition"})
		case strings.Contains(lower, "openapi") || strings.Contains(lower, "swagger"):
			hints = append(hints, BoundaryHint{Type: "http", Path: rel, Details: "API description"})
		case strings.Contains(lower, "kafka"):
			hints = append(hints, BoundaryHint{Type: "kafka", Path: rel, Details: "Kafka-related configuration or code"})
		}
	...
	services := make([]ServiceManifest, 0, len(candidates))
...
func inferServiceBoundaries(hints []BoundaryHint) []BoundaryType {
	seen := map[BoundaryType]struct{}{}
	for _, hint := range hints {
		switch hint.Type {
		case "grpc":
			seen[BoundaryGRPC] = struct{}{}
		case "http":
			seen[BoundaryHTTP] = struct{}{}
		case "kafka":
			seen[BoundaryKafka] = struct{}{}
		}
	}
	result := make([]BoundaryType, 0, len(seen))
...
func addServiceCandidate(candidates map[string]*ServiceManifest, repo Manifest, rootRelPath, entrypoint string) {
	...
	candidates[key] = &ServiceManifest{
		ID:           ServiceID(StableID("svc", repo.RootPath, key, serviceName)),
		Name:         serviceName,
